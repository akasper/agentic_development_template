...
- For every newly opened PR, set exactly one required PR type label (`Bug`, `Feature`, or `Documentation`) in the create step itself. The CI workflow runs label validation first and fails immediately when this requirement is not met.
