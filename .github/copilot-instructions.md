...
- `.github\workflows\plates-address-pr-feedback.yml` monitors `pull_request_review` and `pull_request_review_comment` events. When a known third-party agent (Devin, OpenHands, etc.) leaves feedback on a PR, it posts a structured `@copilot` invocation comment directing Copilot to address the feedback autonomously (apply suggestions, resolve threads, push changes). Configure the agent list via the `PLATE_PR_FEEDBACK_AGENTS` repository variable (comma-separated logins); the workflow falls back to built-in pattern matching when the variable is absent. **Required setup:** create a repository secret named `COPILOT_TRIGGER_PAT` (fine-grained PAT, `pull-requests: write`, owned by a human account) — without it, the trigger comment is posted by `github-actions[bot]` and GitHub will not route it to the Copilot Coding Agent.
